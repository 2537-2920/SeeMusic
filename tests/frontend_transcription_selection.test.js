...
function loadTranscriptionHarness() {
    ...
    vm.runInContext(`
        this.__test = {
            ...
            renderTopbarUser,
            loadCurrentScore,
            ensureEditableScoreForGuitarResult,
            renderGuitarLeadSection,
...
test("loading a stale cached score id clears the local cache after a 404", async () => {
    const { context, api } = loadTranscriptionHarness();
    const storage = new Map();
    context.localStorage.getItem = (key) => (storage.has(key) ? storage.get(key) : "");
    context.localStorage.setItem = (key, value) => storage.set(key, String(value));
    ...
    storage.set("seemusic.transcription.scoreId", "score-missing");
    api.state.selectedScoreId = "score-missing";
    assignOverride(context, "renderControlState", () => {});

    assignOverride(context, "requestJson", async () => {
        const error = new Error("score score-missing not found");
        error.status = 404;
        throw error;
    });

    const result = await api.loadCurrentScore("score-missing", { silent: true });
...
test("guitar lead sections render inline chord flow without missing helper errors", () => {
    const { api } = loadTranscriptionHarness();

    const markup = api.renderGuitarLeadSection(
        {
            role: "verse",
            title: "主歌",
            measureStart: 1,
            ...
            lines: [
                {
                    ...
                    cadence: "open",
                    measureSegments: [
                        { measureNo: 1, chords: [{ symbol: "C", measure_no: 1, beat_in_measure: 1 }] },
                        { measureNo: 2, chords: [{ symbol: "G", measure_no: 2, beat_in_measure: 1 }] },
                    ],
                    tokens: [
                        { type: "bar", measureNo: 1 },
                        { type: "chord", symbol: "C", source: "diatonic", measureNo: 1, beatInMeasure: 1 },
                        { type: "spacer", width: "measure", measureNo: 1 },
                        { type: "bar", measureNo: 2 },
                        { type: "chord", symbol: "G", source: "diatonic", measureNo: 2, beatInMeasure: 1 },
                    ],
                },
            ],
        },
        { highlightedChordSymbol: "G" }
    );

    assert.match(markup, /guitar-lead-line-shell/);
    assert.match(markup, /guitar-inline-chord-line/);
    assert.match(markup, /data-guitar-chord-symbol="C"/);
    assert.match(markup, /data-guitar-chord-symbol="G"/);
});

test("guitar debug panel omits the empty warning placeholder when there are no warnings", () => {
    const { api } = loadTranscriptionHarness();
    api.state.guitarLeadSheetResult = {
        analysis_id: "an_guitar",
        key: "C",
        time_signature: "4/4",
        tempo: 108,
        chords: [{ symbol: "C", source: "diatonic" }],
        warnings: [],
        melody_track: { name: "vocal", source: "separated_track" },
        melody_track_candidates: [],
        separation: { tracks: [] },
        key_detection: { key_signature: "C", mode: "major", fifths: 0, confidence: 0.8, candidates: [] },
        strumming_pattern: { pattern: ["D", "U"] },
        pipeline: { pitch_algorithm: "yin", pitch_source: "separated_track" },
        harmonic_strategy: {
            secondary_dominants_enabled: true,
            borrowed_chords_enabled: true,
            segment_smoothing_enabled: true,
        },
        capo_suggestion: { capo: 2 },
    };
    api.els.guitarDebugPanel = { innerHTML: "" };

    api.renderGuitarDebugPanel();

    assert.doesNotMatch(api.els.guitarDebugPanel.innerHTML, /当前没有额外告警/);
});

test("guitar results create an editable base score when they are generated from pitch data", async () => {
    const { context, api } = loadTranscriptionHarness();
    const calls = [];
    let appliedScore = null;

    assignOverride(context, "ensureScoreOwnerUserId", async () => 42);
    assignOverride(context, "requestJson", async (path, options = {}) => {
        calls.push({ path, options });
        if (path === "/score/from-pitch-sequence") {
            return {
                score_id: "score-guitar-1",
    ...
    assignOverride(context, "applyScoreResult", (score) => {
        appliedScore = score;
    });

    await api.ensureEditableScoreForGuitarResult({
        result: {
            title: "吉他谱",
            analysis_id: "an_guitar",
            tempo: 108,
            time_signature: "4/4",
            key: "C",
            pitch_sequence: [{ time: 0, frequency: 440, duration: 0.5 }],
        },
        base: {
            title: "吉他谱",
            tempo: 108,
    ...
    assert.equal(calls[0].path, "/score/from-pitch-sequence");
    assert.equal(calls[0].options.body.arrangement_mode, "melody");
    assert.equal(calls[0].options.body.user_id, 42);
    assert.equal(appliedScore?.score_id, "score-guitar-1");
});

test("transcription topbar user sync delegates to the shared header helper", () => {
    const { context, api } = loadTranscriptionHarness();
    const calls = [];
    context.window.SeeMusicApp = {
        syncPageUsers(options) {
            calls.push(options);
        },
    };

    api.renderTopbarUser({ username: "Demo User" });

    assert.equal(calls.length, 1);
    assert.equal(calls[0].fallbackName, "游客模式");
    assert.equal(calls[0].fallbackSeed, "SeeMusic");
    assert.deepEqual(calls[0].user, { username: "Demo User" });
});
