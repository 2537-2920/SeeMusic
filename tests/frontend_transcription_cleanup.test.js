...
const appCommonScript = fs.readFileSync(path.join(__dirname, "../frontend/app_common.js"), "utf8");
...
test("transcription header exposes the shared user slot", () => {
    assert.match(html, /data-seemusic-user/);
    assert.match(html, /data-seemusic-user-avatar/);
    assert.match(html, /data-seemusic-user-name/);
    assert.match(script, /function renderTopbarUser/);
    assert.match(script, /syncPageUsers\(/);
});

test("workspace pages use the same shared user header markup", () => {
    [html, communityHtml, singingHtml, profileHtml].forEach((pageMarkup) => {
        assert.match(pageMarkup, /data-seemusic-user/);
        assert.match(pageMarkup, /data-seemusic-user-avatar/);
        assert.match(pageMarkup, /data-seemusic-user-name/);
    });
...
test("singing evaluation visible copy no longer hardcodes WAV wording", () => {
    const singingScript = fs.readFileSync(path.join(__dirname, "../frontend/singing_evaluation.js"), "utf8");
    assert.ok(!singingHtml.includes("上传标准 WAV 音频"));
