...
import backend.services.score_service as score_service_module
...
def test_score_service_round_trips_in_memory_when_db_mode_is_disabled(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(score_service_module, "USE_DB", False)

    created = create_score_from_pitch_sequence(
        {
            "user_id": 9999,
            "title": "In-memory Fallback Score",
            "tempo": 120,
            "time_signature": "4/4",
            "key_signature": "C",
            "pitch_sequence": [{"time": 0.0, "frequency": 440.0, "duration": 0.5}],
    ...
    fetched = get_score(created["score_id"])

    assert created["project_id"] >= 900_000_000
    assert fetched["score_id"] == created["score_id"]
    assert fetched["project_id"] == created["project_id"]
    assert fetched["title"] == "In-memory Fallback Score"
